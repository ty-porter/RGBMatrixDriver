...
class FPSMiddleware:
    @staticmethod
    def inject(driver):
        class _FPSInjectedMatrix(driver.RGBMatrix):
            EMIT_LOG = "log"
            ...
            def __init__(self, *args, **kwargs):
                ...
                self.mode = mode if mode in _FPSInjectedMatrix.EMIT_MODES else None

                if mode == _FPSInjectedMatrix.EMIT_OVERLAY:
            ...
            def __emit_fps(self, canvas):
                if self.mode == _FPSInjectedMatrix.EMIT_LOG:
                    return self.__emit_fps_to_log()
                elif self.mode == _FPSInjectedMatrix.EMIT_OVERLAY:
            ...
            def __emit_fps_to_log(self):
                if self.__should_refresh():
                    self.driver.logger.info(f"FPS: {self.__fps()}")
            ...
            def __emit_fps_to_overlay(self, canvas):
                # Draw background
                for y in range(0, 5):
                    driver.graphics.DrawLine(canvas, 0, y, 18, y, self.bg_color)

                # Draw FPS
                driver.graphics.DrawText(
            ...
            def __load_overlay_mode(self):
                ...
                self.font = driver.graphics.Font()
                self.font.LoadFont(font_path)

                self.text_color = driver.graphics.Color(255, 255, 255)
                self.bg_color = driver.graphics.Color(255, 0, 0)
